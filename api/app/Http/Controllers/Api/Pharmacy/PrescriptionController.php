...
class PrescriptionController extends Controller
{
    ...
    public function updateStatus(Request $request, $id)
    {
        $request->validate([
            ...
            // Médicaments saisis manuellement par le pharmacien lors du devis
            'extracted_medications' => 'nullable|array',
            'extracted_medications.*.name' => 'required_with:extracted_medications|string|max:255',
            'extracted_medications.*.dosage' => 'nullable|string|max:255',
            'extracted_medications.*.quantity' => 'nullable|integer|min:1',
            'extracted_medications.*.unit_price' => 'nullable|numeric|min:0',
            'extracted_medications.*.notes' => 'nullable|string|max:500',
        ]);
        ...
        // Saisie manuelle des médicaments par le pharmacien (remplace l'OCR)
        if ($request->has('extracted_medications')) {
            $prescription->extracted_medications = array_values(array_map(function ($med) {
                return [
                    'name' => trim($med['name']),
                    'dosage' => $med['dosage'] ?? null,
                    'quantity' => (int) ($med['quantity'] ?? 1),
                    'unit_price' => isset($med['unit_price']) ? (float) $med['unit_price'] : null,
                    'notes' => $med['notes'] ?? null,
                    'source' => 'manual',
                ];
            }, $request->input('extracted_medications', [])));
            $prescription->analysis_status = 'manual';
        ...
        // === PROPAGATION VERS LA COMMANDE LIÉE ===
        // Si la prescription est rattachée à une commande, la décision du pharmacien
        // doit faire avancer le cycle de vie de cette commande.
        if ($prescription->order_id) {
            $order = \App\Models\Order::find($prescription->order_id);

            if ($order) {
                if ($request->status === \App\Models\Prescription::STATUS_VALIDATED) {
                    // Ordonnance validée → si déjà payée, confirmer la commande.
                    // Sinon (paiement à la livraison ou non payé), on confirme aussi
                    // car le pharmacien a donné son feu vert sur l'ordonnance.
                    if ($order->status === \App\Models\Order::STATUS_AWAITING_VALIDATION
                        || $order->status === \App\Models\Order::STATUS_PENDING) {
                        $order->markAsValidatedByPharmacist($request->user()->id);

                        \Illuminate\Support\Facades\Log::info('Order confirmed after prescription validation', [
                            'order_id' => $order->id,
                        ...
                        if ($order->customer) {
                            $order->customer->notify(
                                new \App\Notifications\OrderStatusNotification($order, 'confirmed')
                            );
                        }
                    }
                } elseif ($request->status === \App\Models\Prescription::STATUS_REJECTED) {
                    // Ordonnance refusée → annuler la commande.
                    // Si elle était payée, créer une demande de remboursement automatique.
                    $reason = $request->pharmacy_notes
                        ?? $request->admin_notes
                        ?? 'Ordonnance refusée par le pharmacien';

                    $order->markAsRejectedByPharmacist($reason);

                    // Anti-fraude : tracer les rejets (compteur récidive client)
                    try {
                        app(\App\Services\FraudDetectionService::class)->record(
                            type: \App\Models\FraudEvent::TYPE_PRESCRIPTION_REJECTED,
                            userId: $order->customer_id,
                            subject: $prescription,
                            severity: \App\Models\FraudEvent::SEVERITY_MEDIUM,
                            score: 15,
                            payload: [
                                'order_id' => $order->id,
                                'pharmacist_id' => $request->user()->id,
                        ...
                        \Illuminate\Support\Facades\Log::warning('FraudDetection record failed: ' . $e->getMessage());
                    }

                    \Illuminate\Support\Facades\Log::info('Order rejected after prescription refusal', [
                        'order_id' => $order->id,
                        'prescription_id' => $prescription->id,
                        'pharmacist_id' => $request->user()->id,
                        'reason' => $reason,
                    ...
                    // Créer + traiter automatiquement le remboursement si paiement déjà encaissé
                    if ($order->payment_status === 'paid' && $order->payment_mode !== 'cash') {
                        $refund = app(\App\Services\RefundService::class)->createAndAutoProcess(
                            order: $order,
                            reason: 'Ordonnance refusée par le pharmacien : ' . $reason,
                            source: \App\Models\Refund::SOURCE_AUTO_PHARMACIST_REJECT,
                        );

                        \Illuminate\Support\Facades\Log::info('Auto-refund processed after pharmacist rejection', [
                            'order_id' => $order->id,
                            'refund_id' => $refund?->id,
                    ...
                    if ($order->customer) {
                        $order->customer->notify(
                            new \App\Notifications\OrderStatusNotification($order, 'rejected')
                        );
                    }
                }
            }
