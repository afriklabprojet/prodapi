        ...
        if ($duplicate) {
            try {
                app(\App\Services\FraudDetectionService::class)->record(
                    type: \App\Models\FraudEvent::TYPE_PRESCRIPTION_REUSE,
                    userId: $request->user()->id,
                    subject: $duplicate['prescription'],
                    severity: $duplicate['match'] === 'exact'
                        ? \App\Models\FraudEvent::SEVERITY_HIGH
                        : \App\Models\FraudEvent::SEVERITY_MEDIUM,
                    score: $duplicate['match'] === 'exact' ? 40 : 20,
                    payload: [
                        'match' => $duplicate['match'],
                        'distance' => $duplicate['distance'],
                        'previous_prescription_id' => $duplicate['prescription']->id,
                        'previous_status' => $duplicate['prescription']->status,
                    ],
                    request: $request,
                );
            } catch (\Throwable $e) {
                \Log::warning('FraudDetection record failed: ' . $e->getMessage());
            }
        }
