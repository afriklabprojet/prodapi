...
class ProcessPaymentResultJob implements ShouldQueue
{
    ...
    private function processOrderPayment(Order $order, JekoPayment $payment): void
    {
        ...
        // Mettre à jour la prescription liée si elle existe
        ...
        if ($prescription) {
            ...
            // Pour les commandes issues d'une ordonnance déjà préparée par le pharmacien
            // (devis manuel validé + payé), on déclenche directement l'assignation livreur
            // en passant l'order en 'ready' (l'OrderObserver fera l'auto-assign).
            if ($order->status !== 'ready' && $order->status !== 'in_delivery' && $order->status !== 'delivered') {
                $order->status = 'ready';
                $order->save();
                Log::info('ProcessPaymentResult: prescription order auto-marked ready for courier assignment', [
                    'order_id' => $order->id,
