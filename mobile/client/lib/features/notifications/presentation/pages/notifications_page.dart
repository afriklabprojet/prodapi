...
class _NotificationsPageState extends ConsumerState<NotificationsPage> {
  ...
  void _handleNotificationTap(NotificationEntity notification) {
    ...
    // Chat message → navigate directly to chat
    if (shortType == 'chat_message') {
  ...
  /// Ouvre le chat directement depuis une notification de message
  void _openChatFromNotification(Map<String, dynamic> data) {
    final orderId = data['order_id'];
    final deliveryId = data['delivery_id'];
    final senderName = data['sender_name'] as String? ?? 'Expéditeur';
    final senderType = data['sender_type'] as String? ?? 'courier';

    final parsedOrderId = orderId is int ? orderId : int.tryParse(orderId?.toString() ?? '');
    final parsedDeliveryId = deliveryId is int ? deliveryId : int.tryParse(deliveryId?.toString() ?? '');

    // Il faut au moins un order_id ou un delivery_id pour ouvrir le chat
    if (parsedOrderId == null && parsedDeliveryId == null) {
      AppSnackbar.warning(context, 'Impossible d\'ouvrir le chat (données manquantes)');
      return;
    }
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => ChatPage(
          orderId: parsedOrderId ?? 0,
          deliveryId: parsedDeliveryId,
          participantId: 0, // resolved server-side
      ...
      builder: (_) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            // Chat button — for chat_message notifications
            if (shortType == 'chat_message')
              SizedBox(
            ...
            if (isOrderRelated && shortType != 'chat_message')
              SizedBox(
                width: double.infinity,
                child: ElevatedButton.icon(
                  onPressed: () {
