...
class _LoginPageState extends ConsumerState<LoginPage>
  ...
  _mapLoginError(String? error) {
    ...
    // Email/téléphone déjà utilisé (erreur de validation remontée sur login)
    if (errorLower.contains('déjà associé') ||
        errorLower.contains('déjà utilisé') ||
        errorLower.contains('already') ||
        errorLower.contains('taken')) {
      if (errorLower.contains('email') || errorLower.contains('adresse')) {
        return (
          identifierError: useEmail
              ? 'Cet email est déjà associé à un compte. Connectez-vous.'
              : null,
          passwordError: null,
          generalError: useEmail
              ? null
      ...
      return (
        identifierError: null,
        passwordError: null,
        generalError: 'Ce compte existe déjà. Veuillez vous connecter.',
      );
    }
    ...
    // Erreurs de validation (champ requis, format invalide...)
    if (errorLower.contains('veuillez') ||
        errorLower.contains('obligatoire') ||
        errorLower.contains('required') ||
        errorLower.contains('valide')) {
      return (identifierError: null, passwordError: null, generalError: error);
    }

    // Message court et non-technique : l'afficher tel quel
    if (error.length < 100 &&
