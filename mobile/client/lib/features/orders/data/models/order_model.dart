...
class OrderModel {
  ...
  factory OrderModel.fromJson(Map<String, dynamic> json) {
    ...
    // Extract delivery.id → delivery_id for OrderModel
    if (mJson['delivery'] is Map) {
      final delivery = mJson['delivery'] as Map;
      final rawId = delivery['id'];
      if (rawId != null) {
        mJson['delivery_id'] = rawId is String ? int.tryParse(rawId) : rawId;
      }
      // Extract delivery.courier info
      if (delivery['courier'] is Map) {
        final courier = delivery['courier'] as Map;
        final rawCourierId = courier['id'];
        if (rawCourierId != null) {
          mJson['courier_id'] = rawCourierId is String ? int.tryParse(rawCourierId) : rawCourierId;
        }
        mJson['courier_name'] = courier['name'] as String?;
        mJson['courier_phone'] = (courier['phone_courier'] ?? courier['phone']) as String?;
      }
    }
