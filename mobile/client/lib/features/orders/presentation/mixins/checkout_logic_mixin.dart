...
mixin CheckoutLogicMixin<T extends ConsumerStatefulWidget> on ConsumerState<T> {
  ...
  Future<void> _processPayment(int orderId) async {
    ...
    if (selection == null) {
      AppLogger.debug(
        '[Payment] Selection is null, navigating to confirmation without payment',
      );
    ...
    if (provider == 'wallet') {
      ...
      _navigateToConfirmation(orderId, isPaid: success);
      return;
    }
    // ─────────────────────────────────────────────────────────────────────────
    ...
    if (paymentState.status == PaymentStatus.success &&
      ...
      if (paymentUrl.contains('sandbox/confirm') ||
          paymentUrl.contains('localhost')) {
        AppLogger.debug(
          '[Payment] Sandbox mode detected, showing sandbox dialog',
        );
        final confirmed = await _showSandboxPaymentDialog(paymentUrl);
        AppLogger.debug('[Payment] Sandbox dialog result: $confirmed');
        if (mounted) _navigateToConfirmation(orderId, isPaid: confirmed);
      } else {
      ...
      if (mounted) {
        _showCheckoutSnackBar(errorMessage, AppColors.error);
        _navigateToConfirmation(orderId, isPaid: false);
      }
    }
  }
          ...
          ElevatedButton(
            onPressed: () async {
              try {
                ...
                AppLogger.warning(
                  '[Checkout] Sandbox URL launch failed',
                  error: e,
                );
              }
