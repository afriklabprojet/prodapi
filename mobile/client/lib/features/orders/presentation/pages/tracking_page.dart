...
class _TrackingPageState extends ConsumerState<TrackingPage> {
  ...
  // Pharmacy info (pour chat client↔pharmacy)
  int? _pharmacyId;
  ...
  Future<void> _fetchInitialTrackingInfo() async {
    try {
      ...
      if (trackingData != null && mounted) {
        final delivery = trackingData['delivery'] as Map<String, dynamic>?;
        if (delivery != null) {
          _deliveryId = delivery['id'] as int?;
        }

        final courier = delivery?['courier'] as Map<String, dynamic>?
            ?? trackingData['courier'] as Map<String, dynamic>?;
        if (courier != null) {
        ...
        // Récupérer les infos pharmacie
        final pharmacy = trackingData['pharmacy'] as Map<String, dynamic>?;
        if (pharmacy != null) {
          _pharmacyId = pharmacy['id'] as int?;
          _pharmacyName = pharmacy['name'] as String?;
          _pharmacyPhone = pharmacy['phone'] as String?;
        }
    ...
    Navigator.push(
      ...
      MaterialPageRoute(
        builder: (context) => ChatPage(
          ...
          participantId: _courierId!,
          participantName: _courierName!,
          participantType: 'courier',
          participantPhone: _courierPhone,
        ),
      ),
    );
  }

  void _openPharmacyChat() {
    if (_deliveryId == null || _pharmacyId == null || _pharmacyName == null) {
      return;
    }
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => ChatPage(
          orderId: widget.orderId,
          deliveryId: _deliveryId!,
          participantId: _pharmacyId!,
          participantName: _pharmacyName!,
          participantType: 'pharmacy',
          participantPhone: _pharmacyPhone,
        ),
      ),
    );
  }
                      ...
                      firstChild: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            // Chat Pharmacie button
                            if (_deliveryId != null && _pharmacyId != null) ...[
                              const SizedBox(height: 8),
                              SizedBox(
                                width: double.infinity,
                                child: OutlinedButton.icon(
                                  onPressed: _openPharmacyChat,
                                  icon: const Icon(Icons.local_pharmacy, size: 18),
                                  label: Text('💬 Chat avec ${_pharmacyName ?? "la pharmacie"}'),
