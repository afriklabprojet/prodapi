...
class NotificationService {
  ...
  /// Register API client for automatic token refresh sync
    ...
    try {
      ...
      if (settings.authorizationStatus == AuthorizationStatus.authorized ||
        ...
        _messaging.onTokenRefresh.listen((newToken) {
          ...
          // Auto-sync to backend if apiClient is available
          if (_apiClient != null) {
            syncTokenToBackend(_apiClient!);
          }
        });
      }
    } catch (e) {
  ...
  Future<bool> syncTokenToBackend(ApiClient apiClient) async {
    try {
      // Remember apiClient for token refresh auto-sync
      _apiClient = apiClient;
