...
import 'package:drpharma_client/features/addresses/presentation/providers/addresses_provider.dart' show AddressesState, AddressesNotifier, addressesProvider;
...
class MockCartNotifier extends StateNotifier<CartState> with Mock implements CartNotifier {
  MockCartNotifier([CartState? state]) : super(state ?? const CartState.initial());
}

class MockOrdersNotifier extends StateNotifier<OrdersState> with Mock implements OrdersNotifier {
  MockOrdersNotifier() : super(const OrdersState.initial());
}

class MockAddressesNotifier extends StateNotifier<AddressesState> with Mock implements AddressesNotifier {
    ...
    return ProviderScope(
      overrides: [
        ...
        cartProvider.overrideWith((ref) {
          final notifier = MockCartNotifier(cartState ?? const CartState.initial());
          return notifier;
        }),
        ordersProvider.overrideWith((ref) {
          return MockOrdersNotifier()..state = (ordersState ?? const OrdersState.initial());
        }),
        addressesProvider.overrideWith((ref) {
          return MockAddressesNotifier()..state = (addressesState ?? const AddressesState());
        }),
        authProvider.overrideWith((ref) {
          return MockAuthNotifier()..state = (authState ?? const AuthState.initial());
        }),
      ],
      child: MaterialApp(
  ...
  group('CheckoutPage Loading States', () {
    testWidgets('should show loading when placing order', (tester) async {
      await tester.pumpWidget(createTestWidget(
        ...
        ordersState: const OrdersState.initial().copyWith(status: OrdersStatus.loading),
      ));
