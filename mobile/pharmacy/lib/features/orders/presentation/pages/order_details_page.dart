...
import '../widgets/courier_rating_bottom_sheet.dart';
...
class _OrderDetailsPageState extends ConsumerState<OrderDetailsPage> {
  ...
  /// Auto-affiche le bottom sheet de notation du livreur après la livraison.
  /// Protégé par SharedPreferences pour ne s'afficher qu'une seule fois.
  Future<void> _autoShowCourierRating() async {
    if (_order.courierId == null) return;

    final prefs = await SharedPreferences.getInstance();
    final key = 'pharmacy_rated_order_${_order.id}';
    if (prefs.getBool(key) == true) return;

    await Future.delayed(const Duration(milliseconds: 1500));
    if (!mounted) return;

    final result = await CourierRatingBottomSheet.show(
      context,
      orderId: _order.id,
      courierName: _order.courierName ?? 'Livreur',
    );
  ...
  Widget _buildOrderContent(NumberFormat currencyFormat, bool isDark) {
    ...
    return Column(
      ...
      children: [
        ...
        // Customer Info with Chat Button
  ...
  /// Carte client avec bouton de chat intégré
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            // Chat button with client - fonctionne avec orderId OU deliveryId
            if (_order.customerId != null)
              SizedBox(
                width: double.infinity,
                child: ElevatedButton.icon(
                  icon: const Icon(Icons.chat, size: 18),
                  label: Text('💬 Chat avec ${_order.customerName}'),
                  style: ElevatedButton.styleFrom(
                    backgroundColor: Colors.teal,
                    foregroundColor: Colors.white,
                    padding: const EdgeInsets.symmetric(vertical: 10),
                  ...
                  onPressed: () {
                    // Utilise deliveryId si disponible, sinon orderId comme fallback
                    final chatId = _order.deliveryId ?? _order.id;
                    context.push(
                      '/chat',
                      extra: ChatRouteData(
                        deliveryId: chatId,
                        participantType: 'client',
                        participantId: _order.customerId!,
                        participantName: _order.customerName,
                      ),
                    );
                  },
                ),
  ...
  Widget _buildActionButtons(BuildContext context, WidgetRef ref) {
        ...
        child: ActionButtonExpanded(
          ...
          onSuccess: () {
            if (context.mounted) {
              ...
              // Auto-déclencher la notation du livreur (style Uber/Glovo)
              _autoShowCourierRating();
            }
          },
        ),
      );
    ...
    // Bouton "Noter le livreur" pour commandes livrées
    if (_order.status == OrderStatus.delivered &&
        _order.courierId != null &&
        _order.deliveryId != null) {
      return Padding(
        padding: const EdgeInsets.all(16),
        child: SizedBox(
          width: double.infinity,
          child: ElevatedButton.icon(
            ...
            onPressed: () async {
              ...
              if (result == true && mounted) {
                // Optionnel : rafraîchir les détails
              }
            },
          ),
        ),
