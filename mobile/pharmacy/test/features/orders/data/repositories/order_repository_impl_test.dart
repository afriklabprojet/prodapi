...
import 'package:drpharma_pharmacy/core/services/cache_service.dart';
import 'package:drpharma_pharmacy/core/services/offline_storage_service.dart';

class MockOrderRemoteDataSource extends Mock implements OrderRemoteDataSource {}

// Mock CacheService that returns proper values for generic methods
class MockCacheService extends Mock implements CacheService {
  ...
  @override
  T? getData<T>({required String key, T Function(dynamic json)? fromJson}) =>
      null;
}
...
void main() {
  ...
  setUp(() {
    ...
    // Default stub for offlineStorage.queueAction
    when(() => mockOfflineStorage.queueAction(any())).thenAnswer((_) async {});
  });

  group('getOrders', () {
    // Use 'confirmed' status or 'paid' payment to avoid the filter
    // that removes old pending unpaid orders (> 30 min)
    final orderModels = <OrderModel>[
      OrderModel(
        id: 1,
        reference: 'DR-001',
        status: 'pending',
        paymentMode: 'cash', // Cash orders are considered paid, not filtered
        totalAmount: 5000.0,
      ...
      OrderModel(
        id: 2,
        reference: 'DR-002',
        status: 'confirmed', // Not pending, won't be filtered
    ...
    PaginatedOrdersResponse makePaginatedResponse(
    ...
    test(
      'should return paginated orders when remote datasource succeeds',
      () async {
        when(
        ...
        ).thenAnswer((_) async => makePaginatedResponse(orderModels));
        ...
        result.fold((failure) => fail('Should not return failure'), (
          ...
          expect(paginatedResult.orders.length, 2);
          expect(paginatedResult.orders[0].reference, 'DR-001');
          expect(paginatedResult.orders[1].reference, 'DR-002');
        });
      },
    );

    test('should return orders with status filter', () async {
      when(
        () => mockRemoteDataSource.getOrders(
          status: 'pending',
          cursor: any(named: 'cursor'),
      ...
      ).thenAnswer((_) async => makePaginatedResponse([orderModels[0]]));
      ...
      result.fold((failure) => fail('Should not return failure'), (
        ...
        expect(paginatedResult.orders.length, 1);
      });
      verify(
        () => mockRemoteDataSource.getOrders(
          status: 'pending',
    ...
    test('should return NetworkFailure on NetworkException', () async {
      ...
      // MockCacheService.getData returns null by default (cache miss)
  ...
  group('getOrderDetails', () {
    ...
    test('should return order details on success', () async {
      when(
        () => mockRemoteDataSource.getOrderDetails(1),
      ).thenAnswer((_) async => orderModel);

      final result = await repository.getOrderDetails(1);

      expect(result.isRight(), isTrue);
      result.fold((failure) => fail('Should not return failure'), (order) {
        expect(order.id, 1);
    ...
    test('should return ServerFailure on ServerException', () async {
      when(
        () => mockRemoteDataSource.getOrderDetails(1),
      ).thenThrow(ServerException(message: 'Order not found', statusCode: 404));
  ...
  group('confirmOrder', () {
    test('should return Right(null) on success', () async {
      when(
        () => mockRemoteDataSource.confirmOrder(1),
      ).thenAnswer((_) async => {});
    ...
    test(
      'should queue action offline and return Right on NetworkException',
      () async {
        when(
          () => mockRemoteDataSource.confirmOrder(1),
        ).thenThrow(NetworkException(message: 'No internet'));
        ...
        // With offline support, action is queued and returns Right
        expect(result.isRight(), isTrue);
        verify(() => mockOfflineStorage.queueAction(any())).called(1);
      },
    );
  ...
  group('markOrderReady', () {
    test('should return Right(null) on success', () async {
      when(
        () => mockRemoteDataSource.markOrderReady(1),
      ).thenAnswer((_) async => {});
  ...
  group('addNotes', () {
    test('should return Right(null) on success', () async {
      when(
        () => mockRemoteDataSource.addNotes(1, 'Test notes'),
      ).thenAnswer((_) async => {});
