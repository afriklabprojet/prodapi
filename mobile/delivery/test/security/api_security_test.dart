...
void main() {
  ...
  group('URL Security', () {
    test('API URL must use HTTPS', () {
      const apiUrl = 'https://api.drlpharma.pro';
    ...
    test('query parameter sanitization', () {
      ...
      expect(
        sanitizeQueryParam('<script>alert(1)</script>'),
        equals('scriptalert(1)/script'),
      );
  ...
  group('Request Body Security', () {
    ...
    test('content type validation', () {
      final allowedContentTypes = ['application/json', 'multipart/form-data'];
      ...
      expect(
        isAllowedContentType('application/x-www-form-urlencoded'),
        isFalse,
      );
    });
  });
  ...
  group('Response Security', () {
    ...
    test('error responses should not leak info', () {
      ...
      final unsafeErrorMessage =
          'SQLException: SELECT * FROM users WHERE password = ...';
  ...
  group('HTTP Methods Security', () {
    ...
    test('method override prevention', () {
      ...
      expect(
        hasDangerousOverride({'Content-Type': 'application/json'}),
        isFalse,
      );
      expect(
        hasDangerousOverride({'X-HTTP-Method-Override': 'DELETE'}),
        isTrue,
      );
    });
  });
  ...
  group('CORS Security', () {
    test('allowed origins validation', () {
      final allowedOrigins = [
        'https://drlpharma.pro',
        'https://api.drlpharma.pro',
        'https://app.drlpharma.pro',
      ];

      bool isAllowedOrigin(String origin) {
        return allowedOrigins.contains(origin) ||
            origin.endsWith('.drlpharma.pro');
      }

      expect(isAllowedOrigin('https://drlpharma.pro'), isTrue);
      expect(isAllowedOrigin('https://evil.com'), isFalse);
      expect(isAllowedOrigin('https://subdomain.drlpharma.pro'), isTrue);
    });
  });
  ...
  group('Request Timeout Security', () {
    test('timeout values are reasonable', () {
      ...
      expect(
        connectTimeout.inSeconds >= 5 && connectTimeout.inSeconds <= 30,
        isTrue,
      );
      expect(
        receiveTimeout.inSeconds >= 10 && receiveTimeout.inSeconds <= 60,
        isTrue,
      );
      expect(
        sendTimeout.inSeconds >= 10 && sendTimeout.inSeconds <= 60,
        isTrue,
      );
    });
  });
