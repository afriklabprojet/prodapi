...
class ErrorHandler {
  ...
  /// Filtre aussi les messages techniques Laravel non destinés aux utilisateurs
  static String cleanMessage(dynamic error) {
    ...
    // Supprimer le préfixe ServerException: si présent
    if (message.startsWith('ServerException: ')) {
      message = message.substring(17);
    }

    // Supprimer "(status: XXX)" à la fin
    message = message.replaceAll(RegExp(r'\s*\(status:\s*\d+\)$'), '');
  ...
  static String? _extractServerMessage(DioException error) {
    try {
      ...
      if (data is Map) {
        ...
        // Filtrer les messages techniques Laravel non lisibles par l'utilisateur
        if (message != null && _isTechnicalMessage(message)) {
          return null;
        }
        return message;
      }
  ...
  /// Vérifie si un message d'erreur est technique (non lisible par l'utilisateur)
  static bool _isTechnicalMessage(String message) {
    final technicalPatterns = [
      ...
      'Allowed memory size',
      r'\[.*\\.*\]', // Patterns comme [App\Models\Delivery]
    ];
