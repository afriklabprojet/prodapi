...
class EarningsExportService {
  ...
  Future<String?> exportToCSV({
    ...
    try {
      ...
      // Header
      buffer.writeln('ID,Date,Pharmacie,Adresse,Montant Total,Frais Livraison,Statut');
      ...
      buffer.writeln('Total gains,${_currencyFormat.format(totalEarnings)} FCFA');
      
      // Période
      if (dateFrom != null || dateTo != null) {
        buffer.writeln('Période,${dateFrom != null ? _dateFormat.format(dateFrom) : "Début"} - ${dateTo != null ? _dateFormat.format(dateTo) : "Fin"}');
      }
  ...
  Future<String?> exportTransactionsToCSV({
    ...
    try {
      ...
      buffer.writeln('');
      buffer.writeln('RÉSUMÉ');
      buffer.writeln('Total crédits,${_currencyFormat.format(totalCredits)} FCFA');
      buffer.writeln('Total débits,${_currencyFormat.format(totalDebits)} FCFA');
      buffer.writeln('Solde net,${_currencyFormat.format(totalCredits - totalDebits)} FCFA');
  ...
  String generateEarningsReport({
    ...
    buffer.writeln('Date génération: ${_dateTimeFormat.format(DateTime.now())}');
    
    if (dateFrom != null || dateTo != null) {
      buffer.writeln('Période: ${dateFrom != null ? _dateFormat.format(dateFrom) : "Début"} - ${dateTo != null ? _dateFormat.format(dateTo) : "Aujourd'hui"}');
    }
    buffer.writeln('');
    ...
    buffer.writeln('Taux de succès        : ${deliveries.isNotEmpty ? ((delivered / deliveries.length) * 100).toStringAsFixed(1) : 0}%');
    buffer.writeln('');
    
    buffer.writeln('─────────────────────────────────────');
    buffer.writeln('                REVENUS');
    buffer.writeln('─────────────────────────────────────');
    buffer.writeln('Gains bruts           : ${_currencyFormat.format(totalEarnings)} FCFA');
    buffer.writeln('Commissions DR-PHARMA : ${_currencyFormat.format(totalCommissions)} FCFA');
    buffer.writeln('Net à percevoir       : ${_currencyFormat.format(totalEarnings - totalCommissions)} FCFA');
    buffer.writeln('');
    ...
    if (deliveries.isNotEmpty) {
      ...
      for (final delivery in deliveries.where((d) => d.status == 'delivered').take(20)) {
        final date = delivery.createdAt != null 
            ? DateTime.tryParse(delivery.createdAt!) 
            : null;
        buffer.writeln(
          ...
          '${_currencyFormat.format(delivery.deliveryFee)} FCFA'
        );
      }
