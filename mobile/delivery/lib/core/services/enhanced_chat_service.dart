...
class EnhancedChatService {
  ...
  /// UID Firebase de l'utilisateur connecté
  ...
  DocumentReference _conversationRef(int orderId, String target) {
    final uid = _firebaseUid ?? '$_courierId';
    return _firestore
  ...
  Stream<List<EnhancedChatMessage>> watchMessages(int orderId, String target) {
    ...
    if (kDebugMode) {
      debugPrint('💬 [EnhancedChat] watchMessages orderId=$orderId target=$target');
      debugPrint('💬 [EnhancedChat] Firebase user: ${currentUser?.uid ?? "NULL - NOT AUTHENTICATED"}');
    }

    if (currentUser == null) {
      return Stream.error(Exception('Non connecté à Firebase. Veuillez vous reconnecter.'));
    }
